...
This is the central logic of stellar structure.
:::
...
#| warning: false
#| out-width: "100%"
#| fig-cap: "**What to notice:** temperature describes a distribution of particle speeds, not a single particle speed. Higher temperatures broaden the Maxwell-Boltzmann distribution and shift it toward faster particles. This is a pedagogical illustration for protons; the key idea is the changing distribution, not memorizing the formula."
#| fig-alt: "Single plot of normalized Maxwell-Boltzmann speed distributions for protons at 10^4 K, 10^6 K, and 1.5 times 10^7 K. The hotter curves are broader and peak at higher speeds."
...
v = np.linspace(0.0, 1.6e6, 1000)  # m s^-1
...
for temperature, label, color in zip(temperatures, labels, colors):
    distribution = (
        4.0
        * np.pi
        * (m_p / (2.0 * np.pi * k_b * temperature)) ** 1.5
        * v**2
        * np.exp(-m_p * v**2 / (2.0 * k_b * temperature))
    )
    distribution = distribution / distribution.max()
    ax.plot(v / 1000.0, distribution, label=label, color=color, linewidth=3.0)

ax.set_xlabel(r"Speed ($\mathrm{km\,s^{-1}}$)", fontsize=15)
ax.set_ylabel("Relative probability density", fontsize=15)
ax.set_title("Pedagogical Maxwell-Boltzmann Speed Distributions for Protons", fontsize=17, pad=12)
...
This is a **trend estimate**, not a precision stellar-model result. It assumes the simple hydrostatic scalings developed later in the reading, and it suppresses differences in detailed structure and composition. But it captures the key idea we need: radiation pressure becomes disproportionately more important in higher-mass stars.
...
Hydrostatic equilibrium tells us the *slope* of the pressure profile, but not yet the pressure scale itself. To answer our guiding question more quantitatively, we now estimate the central pressure a star must build in order to support its own weight.

### A Scaling Argument

We cannot solve the hydrostatic equilibrium equation exactly without knowing the full internal profiles $M(r)$ and $\rho(r)$. But in astronomy we often know a star’s **global** properties — especially its mass $M$ and radius $R$ — long before we know its detailed interior structure.
...
$$
\frac{P_{c,\text{new}}}{P_{c,\odot}} = \left(\frac{R_\odot}{R_\odot/2}\right)^4 = 2^4 = 16.
$$
...
$$
\left[G\right] = \mathrm{cm^3\,g^{-1}\,s^{-2}}, \quad
\left[M^2\right] = \mathrm{g^2}, \quad
\left[R^4\right] = \mathrm{cm^4}.
$$
...
\frac{G M^2}{R^4}
\rightarrow
\frac{\mathrm{cm^3\,g^{-1}\,s^{-2}}\;\mathrm{g^2}}{\mathrm{cm^4}}
= \mathrm{g\,cm^{-1}\,s^{-2}}.
$$
...
In quasi-static contraction, this relation implies that when gravitational binding energy is released, roughly half goes into increasing the thermal energy of the gas and roughly half must be lost from the star. That bookkeeping is what leads to the counterintuitive thermal behavior of stars.

::: {.callout-tip}
## Check Yourself — Gravitational Scaling
...
Suppose a protostar keeps the same mass but contracts to half its radius.

1. Does $|U_{\text{grav}}|$ increase or decrease?
2. By what factor?
3. What does that imply qualitatively about the thermal energy if the contraction is quasi-static?
...
So shrinking the radius by a factor of $2$ makes the magnitude of the gravitational potential energy larger by a factor of $2$.

That means the star becomes more tightly bound. In quasi-static contraction, the virial theorem implies that the thermal energy also increases, so the core gets hotter.
:::
