...
There is also a useful mass-scaling preview hiding here. The ratio of the two pressure sources is
$$
\frac{P_{\text{rad}}}{P_{\text{gas}}} \propto \frac{T^4}{\rho T} = \frac{T^3}{\rho}.
$$
Later in this reading we will estimate that $T_c \sim M/R$ and $\rho \sim M/R^3$, which implies
$$
\frac{P_{\text{rad}}}{P_{\text{gas}}} \propto \frac{(M/R)^3}{M/R^3} \propto M^2.
$$
This is only a scaling argument, not a precision stellar model, but it captures the trend we care about: radiation pressure becomes disproportionately more important in higher-mass stars.
...
We cannot solve the hydrostatic equilibrium equation exactly without knowing the full internal profiles $\rho(r)$ and $M(r)$. Instead, we estimate the *scale* of the central pressure.

This is not a detailed stellar-structure solution. It is a controlled approximation designed to answer a more modest question: what pressure scale must a star reach in order to support itself?
...
For an order-of-magnitude estimate, replace the local density by the **mean density**:
$$
\rho \sim \frac{M}{R^3}.
$$

> Important: this is a mean-density approximation, not the real density profile of a star. Real stars are centrally concentrated, so this step captures the scaling but not the detailed structure.
...
$$
and replacing $g$ by its global scale $GM/R^2$, we get
$$
...
::: {.callout-tip}
## Check Yourself — Scaling Practice

Suppose a star has:

- twice the mass of the Sun
- the same radius as the Sun
...
1. Predict before calculating: is its central pressure larger or smaller than the Sun’s?
2. By what factor does the central pressure change?
3. Without plugging in solar numbers, check that $GM^2/R^4$ has pressure units in CGS.
...
P_c \propto M^2
$$
at fixed radius, doubling the mass gives
$$
P_c \rightarrow 2^2 P_c = 4P_c.
$$

So the central pressure is **4 times larger**.

The unit check is also reassuring:
$$
\frac{G M^2}{R^4}
\rightarrow
...
That is exactly the unit of pressure in CGS.

Physical reasoning: more mass means stronger gravity, so the star needs a steeper pressure gradient and therefore a larger central pressure.
:::
:::
...
For a bound star, the gravitational potential energy is negative and has the characteristic scale
$$
