...
In older, metal-poor populations the core-helium-burning locus often appears as a horizontal branch; in more metal-rich populations the same core-helium-burning locus may appear instead as a red clump. In both cases, the key inference is that a new central energy source has turned on.
...
For self-gravitating objects, losing energy can make the center hotter, not colder.
...
The turnoff and subgiant observations imply that core hydrogen exhaustion does **not** make the star simply switch off and fade at roughly fixed structure. Instead, they show that gravity regains control of the center, forcing contraction of the inert helium core and heating of the surrounding shell. That is why the star leaves the main sequence and enters subgiant evolution rather than disappearing from the HR diagram.
...
\rightarrow
\text{enhanced outward energy flux through the base of the envelope}
\rightarrow
...
Convection is part of the envelope response, but the deeper point is global structural readjustment: the outer star must find a new equilibrium that can carry the enhanced shell luminosity.
...
An RGB star that is luminous but cool must have a very large radius. That rules out simple fading at roughly fixed radius after core hydrogen exhaustion. Instead, the red giant branch requires large-scale envelope expansion driven by shell burning and transport physics.

**Lock-in:** So the red giant branch is not a star fading after core hydrogen exhaustion; it is the observable signature of shell burning around a contracting inert core driving a much larger envelope structure.
...
This explains why helium ignition happens later: the core must contract much farther than it did during hydrogen burning. But temperature alone is not enough. Helium also faces a nuclear-pathway problem.

#### Why helium must burn through the triple-alpha process

There are no stable nuclei at mass number $A=5$ or $A=8$. So nature cannot climb smoothly upward from helium by single-particle additions. That forces the triple-alpha pathway. The equation below is the **net reaction**:
...
Now that we know how helium burns, we can ask why this stage is comparatively brief.
...
That still leaves one crucial problem. If helium burning finally turns on in the core, why does ignition happen gently in some stars but as a runaway in a low-mass degenerate core?
...
The next question is observational: if the flash is so violent in the core, why do we not see the whole star explode at the surface?
...
The horizontal branch or red clump rules out continued evolution powered only by shell hydrogen burning. It requires a new central energy source, which the model identifies as core helium burning. The helium flash further implies that ignition occurred under degenerate conditions, where the normal thermal thermostat was broken. That is why RGB ascent terminates and the star relocates to the horizontal branch or red clump instead of continuing indefinitely up the RGB.

**Lock-in:** So the horizontal branch or red clump marks the moment a new central energy source has turned on.
...
**Lock-in:** So a white dwarf remains standing not because it is still generating fusion energy, but because quantum mechanics supplies a pressure that does not disappear as the star cools.
...
Low-mass stars do not end by catastrophic collapse, and the HR diagram does not show unrelated classes of stars scattered by chance. It shows a physically connected sequence of structural states that low-mass stars pass through after core hydrogen exhaustion. The observed population signposts therefore imply a sequence of new burning configurations, envelope-loss phases, and finally a degenerate remnant.
...
the physical model lets us infer that these are not unrelated classes of objects, but diagnostic population signposts of the **same low-mass evolutionary pathway**, all triggered by the exhaustion of core hydrogen.
:::
...
| $3\,{^4\text{He}} \rightarrow {^{12}\text{C}} + \gamma$ | Net triple-alpha reaction | Carbon is built from helium |
