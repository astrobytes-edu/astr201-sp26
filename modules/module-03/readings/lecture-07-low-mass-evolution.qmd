...
learning-objectives:
  ...
  - Calculate and compare the energy released per unit mass in hydrogen burning and helium burning
  - Derive how the extreme temperature sensitivity of the triple-alpha process leads to instability in a degenerate helium core
...
5. **Calculate** the energy released per unit mass in hydrogen burning and helium burning and **infer** the consequence for stellar lifetimes.
6. **Derive** how the triple-alpha temperature sensitivity and temperature-independent degeneracy pressure together produce the helium flash.
...
This line is the key to the whole section.
...
The core is now a dense, inert helium core. It is inert because helium fusion requires a much higher temperature than hydrogen fusion.

At the outer edge of that contracting core, a hydrogen-rich layer that was previously too cool for fusion gets compressed and heated. That is where shell burning begins.
...
For proton-proton burning, a useful schematic scaling is
...
4. **That luminosity is deposited at the base of the envelope.**
...
In old clusters, the red giant branch does not continue forever. Stars reach the RGB tip, then populate the horizontal branch. That tells us a new central energy source must turn on. The question is what that source is, and why it behaves so differently from hydrogen burning.
...
But there is another problem. There are no stable nuclei at mass number $A=5$ or $A=8$. So nature cannot climb smoothly upward from helium by single-particle additions. That forces the triple-alpha pathway:
...
That is one reason helium-burning lifetimes are so much shorter.

#### Why the triple-alpha process is unstable in a degenerate core

The triple-alpha energy generation rate has an extremely steep temperature dependence:
...
That is already a very sensitive thermostat in an ordinary gas.
...
with **no explicit temperature dependence**. This scaling assumes the electrons are non-relativistic and degenerate.

That means:

- in an ordinary thermal gas, heating raises pressure and the core expands,
- in a degenerate core, heating does **not** immediately raise the pressure much,
...
This shell structure is thermally unstable. The helium shell can ignite in pulses, causing:
...
the physical model tells us we are seeing different stages of the **same low-mass evolutionary pathway**, all triggered by the exhaustion of core hydrogen.
:::
