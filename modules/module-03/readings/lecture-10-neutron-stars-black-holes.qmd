...
learning-objectives:
  ...
  - State the TOV limit (about 2 to 3 solar masses) and explain why it represents the maximum neutron star mass
...
Concept-Throughline:
  ...
  - The TOV limit (about 2 to 3 solar masses) is the neutron star analogue of the Chandrasekhar limit — above it, nothing in known physics can resist gravity
  - General relativity replaces Newtonian gravity when speeds approach $c$ or when the Schwarzschild radius approaches the physical radius
...
| Magnetic field | $10^{12}\text{–}10^{15}~\text{G}$ | trillions to quadrillions of times Earth's field |
| Rotation period | $\sim 0.001\text{–}10~\text{s}$ | Up to 716 Hz |

### The Density Calculation

A teaspoon ($\sim 5~\text{cm}^3$) of neutron star material would have a mass of:
...
m
&=
\left(5 \times 10^{14}~\mathrm{g\,cm^{-3}}\right)\left(5~\mathrm{cm^3}\right) \\
&=
...
&=
2.5 \times 10^{12}~\mathrm{kg}
\left(\frac{1~\text{tonne}}{10^3~\mathrm{kg}}\right) \\
&=
2.5 \times 10^9~\text{tonnes}
\end{aligned}
$$

$$
\boxed{m = 2.5 \times 10^9~\text{tonnes}}
$$

That's $2.5$ billion tonnes of mass — packed into a teaspoon-sized volume. This is a scale analogy, not a literal laboratory scenario: neutron-star matter is stable only under extreme pressure, so a real "spoonful" could not simply sit unchanged on Earth.
...
$$
\frac{P_1}{P_2} = \frac{\omega_2}{\omega_1} = \frac{R_1^2}{R_2^2}.
$$
...
&=
\left(\frac{7 \times 10^{10}~\text{cm}}{10^6~\text{cm}}\right)^2 \\
&=
...
\begin{aligned}
25~\text{days}
\left(\frac{86400~\text{s}}{1~\text{day}}\right)
&=
2.16 \times 10^6~\text{s}
\end{aligned}
$$
...
&=
\frac{2.16 \times 10^6~\text{s}}{4.9 \times 10^9} \\
&\approx
4.4 \times 10^{-4}~\text{s}
\end{aligned}
$$
...
\begin{aligned}
B_1 R_1^2 &= B_2 R_2^2 \\
\Rightarrow \quad B_2 &= B_1 \left(\frac{R_1}{R_2}\right)^2
\end{aligned}
$$
...
B_2
&=
1~\text{G} \times \left(\frac{7 \times 10^{10}~\text{cm}}{10^6~\text{cm}}\right)^2 \\
&=
4.9 \times 10^9~\text{G}
\end{aligned}
$$
...
The pulse period equals the neutron star's rotation period. Pulsars are observed with periods ranging from milliseconds to seconds. Over time, many pulsars gradually slow down because they lose rotational energy through their magnetic fields and particle outflows. By measuring how the period changes, astronomers can infer useful physical information such as the pulsar's magnetic field strength and an estimate of its age.

In general, ordinary isolated pulsars spin more slowly as they age. However, some old neutron stars in binary systems can later be spun back up by accreting matter from a companion; these are the millisecond pulsars.

::: {.callout-tip}
## Check Yourself

Why does a pulsar's extremely short pulse period tell us it must be a compact object rather than a normal star?

::: {.callout-note collapse="true"}
## Solution

A star with a radius like the Sun's cannot rotate hundreds of times per second without flying apart. Millisecond-to-second pulse periods therefore require a much smaller object with a radius of only tens of kilometers. That is one of the key clues that pulsars are neutron stars.
:::
:::
...
**A black hole is not an object in space — it is a region of spacetime from which nothing, not even light, can escape to distant observers.** A useful Newtonian preview is that the Schwarzschild radius is the radius where the classical escape speed would equal $c$. In general relativity, the event horizon is more fundamentally the boundary beyond which no future-directed signal can escape outward.
:::
...
This is the **Schwarzschild radius**. In the Newtonian preview, it is the radius at which the classical escape speed equals the speed of light. In general relativity, the associated event horizon is the boundary beyond which no outward signal can reach distant observers.
...
\frac{2GM_\odot}{c^2} \\
&=
\frac{2(6.67 \times 10^{-8}~\mathrm{cm^3\,g^{-1}\,s^{-2}})(2.0 \times 10^{33}~\mathrm{g})}{(3.0 \times 10^{10}~\mathrm{cm\,s^{-1}})^2} \\
&=
\frac{2.67 \times 10^{26}~\mathrm{cm^3\,s^{-2}}}{9.0 \times 10^{20}~\mathrm{cm^2\,s^{-2}}} \\
&\approx 3.0 \times 10^5~\mathrm{cm}
\end{aligned}
$$

$$
\boxed{R_{s,\odot} = 3.0~\text{km}}
$$
...
\frac{2GM_\oplus}{c^2} \\
&=
\frac{2(6.67 \times 10^{-8}~\mathrm{cm^3\,g^{-1}\,s^{-2}})(6.0 \times 10^{27}~\mathrm{g})}{(3.0 \times 10^{10}~\mathrm{cm\,s^{-1}})^2} \\
&\approx 0.89~\mathrm{cm}
\end{aligned}
$$

$$
\boxed{R_{s,\oplus} \approx 0.89~\text{cm}}
$$
...
A neutron star has $M = 1.4\,M_\odot$ and $R = 10~\text{km}$. Calculate the ratio $R_s/R$ and explain what it tells us about how relativistic the neutron star is.
...
\frac{R_s}{R_\odot}
&=
\frac{3~\text{km}}{7 \times 10^5~\text{km}} \\
&=
4 \times 10^{-6}
\end{aligned}
$$
...
R_s
&=
3.0~\text{km} \times 4 \times 10^6 \\
&=
1.2 \times 10^7~\text{km}
\end{aligned}
$$

$$
\boxed{R_s = 1.2 \times 10^7~\text{km}}
$$
...
\frac{R_s}{R_\odot}
&=
\frac{1.2 \times 10^7~\text{km}}{7 \times 10^5~\text{km}} \\
&\approx 17
\end{aligned}
$$
...
\frac{R_s}{1~\text{AU}}
&=
\frac{1.2 \times 10^7~\text{km}}{1.5 \times 10^8~\text{km}} \\
&=
0.08
\end{aligned}
$$
...
| Surface? | Yes (material surface / thin atmosphere) | Yes (solid crust over dense interior) | No material surface; event horizon instead |
...
1. **Neutron stars are nuclear-density objects** — about $1.4\,M_\odot$ packed into a radius of about $10~\text{km}$, spinning from milliseconds to seconds, with magnetic fields of $10^{12}\text{–}10^{15}~\text{G}$, vastly stronger than Earth's. They are held up by neutron degeneracy pressure — the last stand of quantum mechanics against gravity.
