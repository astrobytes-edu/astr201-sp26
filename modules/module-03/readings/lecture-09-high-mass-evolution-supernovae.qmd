...
| Stage | Fuel → Product | $T_\text{ignition}$ | Duration ($25\,M_\odot$ star) |
|:------|:--------------|:------|:-------|
...
$$
\frac{\tau_\text{Si}}{\tau_\text{H}} \sim \frac{1~\text{day}}{7 \times 10^6~\text{yr}} \sim 4 \times 10^{-10}
$$
...
- **Fusing elements lighter than iron** → products are more tightly bound → energy is released ✓
- **Fusing elements heavier than iron** → products are *less* tightly bound → energy is *consumed* ✗
...
Since $R_f = 10~\text{km} = 10^6~\text{cm}$ and $R_i = 1{,}000~\text{km} = 10^8~\text{cm}$:

$$
\frac{1}{R_f} - \frac{1}{R_i} = \frac{1}{10^6} - \frac{1}{10^8} = 10^{-6}(1 - 0.01) \approx 10^{-6}~\text{cm}^{-1}
$$

The $1/R_i$ term contributes only $1\%$ — the final radius dominates. The energy released:

$$
\Delta E \sim \frac{GM^2}{R_f} = \frac{6.67 \times 10^{-8} \times (2.8 \times 10^{33})^2}{10^6}
$$
...
| Stage | Fuel → Ash | $T$ (K) | $\rho$ (g/cm³) | Duration | Energy (MeV/nucleon) |
|:------|:-----------|:--------|:------|:---------|:---------|
...
│      GRAVITY WINS COMPLETELY. → Black hole.          │
│                                                      │
...
│  Next: Neutron stars & black holes → Reading 10      │
└──────────────────────────────────────────────────────┘
```
