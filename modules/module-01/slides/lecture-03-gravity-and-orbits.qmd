...
- **Eccentricity** $e$:<br> how "squashed"<br> (0 = circle, 1 = line)
:::
:::
...
{{< img kepler-second-law width="80%" >}}
...
::: {.incremental}
- Move a planet 2× farther → period increases by $2^{3/2} \approx 2.83$
- It's a *proportionality*, not an equality
- The constant depends on the central mass (Newton will show this)
:::
...
The "shortcut" only works for Solar System orbits measured in years and AU around a ~$1\,M_\odot$ star.
...
**Before I show you:** If you move a planet to *twice* its orbital distance, by what factor does its period increase?
...
::: {.notes}
Give 30 seconds. Answer: **C**. From $P^2 \propto a^3$: if $a$ doubles, $P^2 \to 2^3 = 8$, so $P \to \sqrt{8} \approx 2.83$.
:::
...
{{< img newton-ball-on-string width="100%" >}}

::: {.fragment}
**Centripetal acceleration:** $a_c = \dfrac{v^2}{r}$ (points toward center)
:::
...
::: {.notes}
Press **C** to reveal. Common misconception: confusing speed with velocity.
:::
...
::: {.notes}
Newton's big insight: one law explains both falling apples and orbiting planets.
:::
...
::: {.incremental}
- $G = 6.67 \times 10^{-8}\ \mathrm{cm^3\,g^{-1}\,s^{-2}}$ — gravitational constant
- $m_1, m_2$ — the two masses (in grams)
- $r$ — distance between centers (in cm)
- $F_g$ — force (in dynes), attractive
:::

. . .

**Unit check:** $[G] = \dfrac{\text{dyne} \cdot \text{cm}^2}{\text{g}^2} = \mathrm{cm^3\,g^{-1}\,s^{-2}}$ ✓

::: {.notes}
Walk through the units. This verifies the equation makes dimensional sense.
:::
...
::: {.notes}
30 seconds. Answer: **B**. Inverse-square: $(2r)^2 = 4r^2$, so force is 1/4.
:::
...
Armed with $F = GMm/r^2$, Newton derived:
...
::: {.incremental}
- The constant is now *determined*: depends on $G$ and total mass
- Both masses appear: $(M + m)$
- Applies to *any* gravitational system, not just our solar system
:::

::: {.notes}
This is how we "weigh" stars in binaries, black holes, galaxies.
:::
...
::: {.fragment}
When $M \gg m$: barycenter ≈ center of $M$ (Kepler's approximation works)

This wobble is how we detect **exoplanets**!
:::

::: {.notes}
Connect to radial velocity method. The Sun wobbles ~1 solar radius due to Jupiter.
:::
...
**Given:** $M_\odot = 2 \times 10^{33}$ g, $r = 1.5 \times 10^{13}$ cm, $G = 6.67 \times 10^{-8}$

. . .

$$v_{orb} = \sqrt{\frac{GM_\odot}{r}} = \sqrt{\frac{(6.67 \times 10^{-8})(2 \times 10^{33})}{1.5 \times 10^{13}}}$$

. . .

$$= \sqrt{8.9 \times 10^{12}} \approx 3 \times 10^6\ \text{cm/s} = 30\ \text{km/s}$$

::: {.notes}
Walk through the calculation showing unit cancellation.
:::
...
::: {.notes}
Press **C**. From $v_{orb} \propto 1/\sqrt{r}$: Earth is $\sqrt{5} \approx 2.2$ times faster.
:::
...
::: {.incremental}
- $K = \frac{1}{2}mv^2$ — kinetic energy (always positive)
- $U = -\frac{GMm}{r}$ — potential energy (always negative)
:::

::: {.notes}
Students often confused by negative PE. Address this next.
:::
...
The deeper in the well → the more negative $U$ → the more you need to climb out.

::: {.notes}
The debt metaphor really helps students. Relate to energy budget.
:::
...
**Two equivalent forms:** $v = r\dot{\phi}$ (arc length per time), so both give the same $L$.
...
::: {.notes}
Torque = what makes things spin faster/slower. Central force → no torque → L conserved. $\dot{\phi}$ = angular velocity (rad/s).
:::
...
::: {.notes}
30 seconds. Answer: **B**. From $v_p r_p = v_a r_a$: $v_p/v_a = r_a/r_p = 4/1 = 4$.
:::
...
For circular orbit (instant = average): $K = \dfrac{GMm}{2r}$, $U = -\dfrac{GMm}{r}$ → $K = -\frac{1}{2}U$ ✓
...
3. **Force balance** → $v_{orb} = \sqrt{GM/r}$
4. **Energy conservation** → $v_{esc} = \sqrt{2GM/r}$; $E$ determines bound vs. unbound
...
| Escape Velocity | $v_{esc} = \sqrt{\dfrac{2GM}{r}} = \sqrt{2}\,v_{orb}$ |
| Newton's Kepler III | $P^2 = \dfrac{4\pi^2}{G(M+m)}a^3$ |
| Total Energy | $E = \dfrac{1}{2}mv^2 - \dfrac{GMm}{r}$ |
| Bound Orbit Energy | $E = -\dfrac{GMm}{2a}$ |
| Angular Momentum | $L = mvr = mr^2\dot{\phi}$ |
| Virial Theorem | $\langle K \rangle = -\frac{1}{2}\langle U \rangle$ |
