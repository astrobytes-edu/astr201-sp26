...
**Write as equality with constant $C$:** For any object orbiting the same central mass:
$P = C \cdot a^{3/2}$

. . .

**For two objects (1 and 2) orbiting the same mass:**
$P_1 = C \cdot a_1^{3/2} \quad \text{and} \quad P_2 = C \cdot a_2^{3/2}$

. . .

**Divide to eliminate $C$:**
$$\frac{P_2}{P_1} = \frac{\cancel{C} \cdot a_2^{3/2}}{\cancel{C} \cdot a_1^{3/2}} = \left(\frac{a_2}{a_1}\right)^{3/2}$$

::: {.text-xs .text-muted}
**Why this works:** Proportionality ($\propto$) hides an unknown constant. When you divide two proportionalities *with the same constant*, it cancels—leaving a true equality ($=$).
:::

::: {.notes}
Walk through every step. Students struggle with proportionality → equality → ratio. Emphasize: the ratio method turns "$\propto$" into "$=$" by canceling what we don't know.
:::
